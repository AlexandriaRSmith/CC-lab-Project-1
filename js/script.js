...
var pipes;
var gameOver;
var birdImg, obImg, groundImg, bgImg;


function setup() {
  createCanvas(400,600);

  birdImg = loadImage("assets/penguin.png");
  obImg = loadImage("assets/planet3.png");
...
function draw() {
  ...
  if(!gameOver) {
    ...
    //make obsticles
    if(frameCount%60 == 0) {
      var pipeH = random(800, 100);
      var pipe = createSprite(bird.position.x + width, GROUND_Y-pipeH+1+100, 80, pipeH);
      pipe.addImage(obImg);
      pipes.add(pipe);

    }

    //get rid of obsticles
